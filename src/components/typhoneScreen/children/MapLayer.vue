<template>
    ...
    <div class="right-tools-btns">
      ...
      <!-- <div class="panorama-control">
        <div
          :class="{ 'active-panorama-btn-box': panoramaControlFlag }"
          class="btn-box"
          @click="addPanoramaImage"
        ></div>
        全景图
      </div> -->
      <!-- 指北针  仅占位使用 -->
  ...
  export default {
    ...
    data() {
      return {
        panoramaControlFlag: false,
        drawerHover: false,
        baseMapList: ["夜景地图", "倾斜摄影", "影像地图", "二维夜景"],
        activeBaseMap: "",
      };
    },
    computed: {
    ...
    methods: {
      ...
      // 切换底图
      changeBaseMap(type) {
        this.$bus.$emit("pagechangeImageFunctionByName", type);
        this.activeBaseMap = type;
      },
      // 添加全景图
      addPanoramaImage() {
        this.panoramaControlFlag = !this.panoramaControlFlag;
        this.$bus.$emit("pageisAddPhotoSherePointList", this.panoramaControlFlag);
      },
    },
  };
  </script>
