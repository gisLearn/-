...
// 导入三角面片绘制
import { PrimitiveTriangles } from "@/components/typhoon/utils/Tools-18PrimitiveTriangles.js"
import { EllipseGeometryLibraryEx } from "@/components/typhoon/utils/Tools-21EllipseGeometryLibraryEx.js"
/**
 *
 *台风对象初始化
 * @param {*} viewer 地球类型状态
 ...
 */
export var typhoon = function (viewer, Cesium, typhoonData, ForecastPathStatus, typhoonStatusinfo) {
    this.viewer = viewer;
    this.Cesium = Cesium;
    ...
    this.tylabelenetity = null;//台风label标签
            ...
            if (that.count < that.pointlength) {
                ...
                var infopoint = {
                    ...
                    properties: {
                        ...
                        name: drawinfo.strong,
                        time: drawinfo.time,
                        power: drawinfo.power,
                        speed: drawinfo.speed,
                        pressure: drawinfo.pressure,
                        radius7: drawinfo.radius7,
                        radius10: drawinfo.radius10,
                        radius12: drawinfo.radius12,
                        isCurrent: that.isCurrent
        ...
        for (let index = 0; index < that.mainlandForecastMaxCount; index++) {
            if (index != 0) {
                let infopoint = {
                    ...
                    lon: "",
                    lat: "",
                    imgSrc: require("@/assets/image/typhoon/typhoonImage/blankImage.png"),
                    properties: {
                        organ: "中国",
                        pressure: 0,
                        speed: 0,
    ...
    if (that.mainlandForecastMaxCount != 0 && mainlandForecastList) {
        for (let index = 0; index < that.mainlandForecastPointEntities.length; index++) {
            ...
            if (infoPoint) {
                ...
                entitiesPoint._billboard._image._value = require("@/assets/image/typhoon/typhoonImage/blankImage.png");
...
function TyphoonImageRank(typhoon_Rank_name) {
    ...
    switch (typhoon_Rank_name) {
        case "热带风暴":
            typhoon_point_image = require("@/assets/image/typhoon/typhoonImage/Tropical_Storm_Point_image.png");
            break;

        case "台风":
            typhoon_point_image = require("@/assets/image/typhoon/typhoonImage/Typhoon_Point_image.png");
            break;

        case "强台风":
            typhoon_point_image = require("@/assets/image/typhoon/typhoonImage/Violent_Typhoon_Point_image.png");
            break;

        case "超强台风":
            typhoon_point_image = require("@/assets/image/typhoon/typhoonImage/Super_Typhoon_Point_image.png");
            break;

        case "强热带风暴":
            typhoon_point_image = require("@/assets/image/typhoon/typhoonImage/Server_Tropical_Storm_Point_image.png");
            break;

        case "热带低压":
            typhoon_point_image = require("@/assets/image/typhoon/typhoonImage/Tropical_Depression_Point_image.png");
            break;
        default:
            typhoon_point_image = require("@/assets/image/typhoon/typhoonImage/Typhoon_Point_image.png");
    }
...
typhoon.prototype.addBillboardRotation = function (info) {
    ...
    var entity = that.viewer.entities.add({
        ...
        billboard: {
            image: require("@/assets/image/typhoon/typhoonImage/cirecleImage.png"),
...
// 台风标签
typhoon.prototype.CreateTyphoonHtml = function () {
    var that = this;


    let typhoonMarkerWorld = that.Cesium.Cartesian3.fromDegrees(that.firstPointlon, that.firstPointlat)
    let tyname = that.typhoonName;
    that.tylabelenetity = that.viewer.entities.add({
        name: '标点',
        position: typhoonMarkerWorld,
        label: { //文字标签
            text: `${tyname}`,
...
typhoon.prototype.removeTyphoonHtml = function () {
    var that = this;
    if (that.tylabelenetity) {
        that.viewer.entities.remove(that.tylabelenetity)
        that.tylabelenetity = null;
    }
}
 ...
 * isCurrent 是否为当前台风
 */
export function typhoonPopup(drillPick, movement, viewer) {
    let infotyphoon = {}
    // var inClick = false;
    // typhoonInfoDelete();
    for (var i = 0; i < drillPick.length; i++) {
        var pick = drillPick[i]
        if (pick && pick.id && pick.id.id && (pick.id.name === '热带风暴' || pick.id.name === '热带低压' || pick.id.name === '强热带风暴' || pick.id.name === '台风' || pick.id.name === '强台风' || pick.id.name === '超强台风') && pick.id._properties) {
            var clickEntity = pick.id
            // var movement_pick = movement.position
            let tyCartesian = clickEntity.position._value
            ...
            let lat = clickEntity.properties.lat._value;//纬度
            let lon = clickEntity.properties.lon._value;//经度
            let number = clickEntity.properties.number._value;//编号
            let tyType = clickEntity.properties.name._value;//台风类型

            let power = clickEntity.properties.power._value;//风力强度
            let pressure = clickEntity.properties.pressure._value;//气压
            let radius7 = clickEntity.properties.radius7._value || "0";

            let radius10 = clickEntity.properties.radius10._value || "0";
            let radius12 = clickEntity.properties.radius12._value || "0";
            let speed = clickEntity.properties.speed._value;
            let time = clickEntity.properties.time._value;

            let typhoonName = clickEntity.properties.typhoonName._value;
            let info = {
                tyCartesian: tyCartesian,
                "台风名称": typhoonName,
                "经度": lon,
                "纬度": lat,
                "台风标号": number,
            ...
            infotyphoon.properties = info;

        } else if (pick && pick.id && pick.id.id && (pick.id.name === '预测点')) {
            var clickEntity = pick.id
            // var movement_pick = movement.position
            let tyCartesian = clickEntity.position._value

            // typhoneInfoShow(leftX, topY, typhoonNumber, typhoonName, typhoonId, typhoonDistance, isCurrent, false, typhoonFlag)
            let organ = clickEntity._properties._organ._value
            let lng = clickEntity._properties._lng._value
            let lat = clickEntity._properties._lat._value
            let power = clickEntity._properties._power._value
            let pressure = clickEntity._properties._pressure._value
            let speed = clickEntity._properties._speed._value
            let strong = clickEntity._properties._strong._value
            let time = clickEntity._properties._time._value
            let info = {
                tyCartesian,
                "纬度": lat,
                "经度": lng,
