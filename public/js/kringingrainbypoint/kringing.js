...
var kriging = function () {
    ...
    kriging.plot = function (canvas, grid, xlim, ylim, colors) {
        // 获取计算颜色区间数据
        let colorarry = kriging.getColors(colors);
        ...
        for (i = 0; i < n; i++) {
            for (j = 0; j < m; j++) {
                ...
                ctx.fillStyle = kriging.getColor(colors, grid[i][j], colorarry);
    ...
    //自定义色彩将数组中的值按照颜色长度分成多少区域，在区域范围
    // 插值取出对应值的颜色范围
    kriging.getColor = function (colors, z, colorarry) {


        for (const key in colorarry) {
            if (colorarry.hasOwnProperty(key)) {
                const element = colorarry[key];
                if (z > element[1] && z < element[0]) {
        ...
        //         let colorarray = colors[i].color;

        //         let step = colorarray.length;
        //         for (let index = step; index > 0; index--) {
        //             // 渐变等值
        //             let stepvalue = (colors[i].max - colors[i].min) / step;
        //             let minxvalue = colors[i].min + (index * stepvalue);
        //             let manxvalue = colors[i].min + ((index - 1) * stepvalue);
        //             colorobj[colorarray[step - index]] = [minxvalue, manxvalue]
        //         }
    ...
    // 克里金插值源码色值修改
    kriging.getColors = function (colors) {
